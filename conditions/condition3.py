...
def run(csv_path: str, source: str, n_clusters: int = 20) -> ConditionArtifacts:
    ...
    # LLM labeling: one call per cluster
    ...
    t0 = time.time()
    report, gen_in, gen_out = generate("Hierarchical Clustering + LLM Labels", entries, clusters, lfe_list)
    ...
    metrics = RunMetrics(
        ...
        llm_calls=len(clusters) + 1,  # one per cluster label + one for generate
