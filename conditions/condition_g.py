...
def run(csv_path: str, source: str, n_clusters: int = 20) -> ConditionArtifacts:
    ...
    t0 = time.time()
    report, gen_in, gen_out = generate("G: Hierarchical + Keywords", entries, clusters, [])
    gen_secs = time.time() - t0

    total_secs = time.time() - total_start

    llm_secs = gen_secs + (embed_secs if embed_tokens > 0 else 0.0)
    preprocessing_secs = cluster_secs + kw_secs + (0.0 if embed_tokens > 0 else embed_secs)
    ...
    return ConditionArtifacts(
        condition="G: Hierarchical + Keywords",
        corpus_id=source,
